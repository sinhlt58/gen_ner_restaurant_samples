...
generated_queries_file = codecs.open("train/p_gen_queries.txt", "w", "utf-8")
#labled file
tagged_train_file = open("train/uet-ner.train", "w")
			...
			if not line == "\n":
				...
				for token in tokens:
					if len(token.split())==1:
						token = token + '\t' + 'O'
					tagged_train_file.write(token+'\n')
				tagged_train_file.write('\n')
...
seed_variables = {}
for f in os.listdir('./p_variables'):
	duplicated_vars.append(f)
	with open('p_variables/' + f) as data_f:
		seed_variables[f] = data_f.readlines()	
gen_queries(template_queries, duplicated_vars, seed_variables)
generated_queries_file.close()
label("train/p_gen_queries.txt", seed_variables)
template_queries_file.close()

generated_queries_file = codecs.open("train/n_gen_queries.txt", "w", "utf-8")
...
#load variable values in p_variables folder
seed_variables = {}
for f in os.listdir('./n_variables'):
	duplicated_vars.append(f)
	with open('n_variables/' + f) as data_f:
		seed_variables[f] = data_f.readlines()	
gen_queries(template_queries, duplicated_vars, seed_variables)
generated_queries_file.close()
label("train/n_gen_queries.txt", seed_variables)
template_queries_file.close()

tagged_train_file.close()
#train
subprocess.call(['java', '-cp', 'stanford-ner.jar', 'edu.stanford.nlp.ie.crf.CRFClassifier', '-prop', 'crf-g.prop'])